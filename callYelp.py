...
restaurant = {'ak':18,'al':43,'ar':29,'az':324 ,'ca':679 ,'co':100 ,'ct':23,'de':21,'fl':214 ,'ga':369 ,'hi':102 ,'ia':18,'id':24,'il':368 ,'in':87,'ks':25,'ky':61,'la':680 ,'ma':175 ,'md':67,'me':21,'mi':231 ,'mn':33,'mo':83,'ms':12,'mt':9,'nc':96,'nd':11,'ne':33,'nh':18,'nj':70,'nm':59,'nv':207 ,'ny':1122,'oh':110 ,'ok':61,'or':211 ,'pa':148 ,'ri':55,'sc':25,'sd':10,'tn':42,'tx':201 ,'ut':45,'va':46,'vt':15,'wa':173 ,'wi':44,'wv':9,'wy':7}
...
def do_test(term, loc, x=0):
    ...
    while x<y:
        try:
            f = open(logFile, 'a')
            url_params = '-l="'+loc+'" -q="'+term+'" --offset='+str(x)
            #Log
            f.write(loc+'\t'+str(x)+'\n')
            #Json
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 3:
        offset = sys.argv[2]
        processes = [Process(target=do_work, args=(work_queue, term, loc, offset)) for loc in state]
    else:
